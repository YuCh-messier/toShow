...
var TableTreeStore = /*#__PURE__*/function () {
  ...
  createClass._createClass(TableTreeStore, [{
    ...
    value: function getAllUniqueKeys(data, keys) {
      ...
      for (var i = 0, len = data.length; i < len; i++) {
        var item = data[i];
        arr.push(get.get_1(item, keys.rowKey));
        var children = get.get_1(item, keys.childrenKey);
    ...
    value: function toggleExpandData(p, dataSource, keys) {
      if (!p) {
        _common_js_log_log["default"].error("EnhancedTable", "the node you want to toggleExpand doest not exist in `data`");
        return dataSource;
      }

      var rowValue = get.get_1(p.row, keys.rowKey);
      ...
      var childrenNodes = get.get_1(p.row, keys.childrenKey);
    ...
    value: function updateExpandRow(changeRow, dataSource, keys) {
      ...
      var childrenNodes = get.get_1(row, keys.childrenKey);
    ...
    value: function updateData(rowValue, newRowData, dataSource, keys) {
      var newRowValue = get.get_1(newRowData, keys.rowKey);
      ...
      if (get.get_1(newRowData, keys.childrenKey) === true) {
        var oldChildren = get.get_1(rowState.row, keys.childrenKey);

        if (oldChildren !== null && oldChildren !== void 0 && oldChildren.length) {
          for (var i = 0, len = oldChildren.length; i < len; i++) {
            var rowValue2 = get.get_1(oldChildren[i], keys.rowKey);
      ...
      if (rowState.parent) {
        var siblings = get.get_1(rowState.parent.row, keys.childrenKey);
        var index = siblings.findIndex(function (item) {
          return get.get_1(item, keys.rowKey) === rowValue;
        });
    ...
    value: function remove(key, dataSource, keys) {
      ...
      if (r && r.rowIndex >= 0) {
        ...
        if (r.parent) {
          var siblings = get.get_1(r.parent.row, keys.childrenKey);
          var index = siblings.findIndex(function (item) {
            return get.get_1(item, keys.rowKey) === key;
          });
    ...
    value: function appendTo(rowValue, newData, dataSource, keys) {
      ...
      var children = get.get_1(state.row, keys.childrenKey);
      ...
      for (var i = 0, len = tmpData.length; i < len; i++) {
        ...
        var newRowValue = get.get_1(oneData, keys.rowKey);
    ...
    value: function appendToRoot(newData, dataSource, keys) {
      ...
      for (var i = 0, len = tmpNewData.length; i < len; i++) {
        var rowValue = get.get_1(tmpNewData[i], keys.rowKey);
    ...
    value: function insert(rowValue, newData, dataSource, keys, type) {
      var state = this.treeDataMap.get(rowValue);
      if (!this.validateDataExist(state, rowValue)) return dataSource;
      var newRowValue = get.get_1(newData, keys.rowKey);
      var mapState = this.treeDataMap.get(newRowValue);
      ...
      if (state.parent) {
        var childrenIndex = state.parent.row[keys.childrenKey].findIndex(function (t) {
          return rowValue === get.get_1(t, keys.rowKey);
        });
    ...
    value: function swapData(dataSource, params, keys) {
      ...
      var startRowValue = get.get_1(params.current, keys.rowKey);
      var endRowValue = get.get_1(params.target, keys.rowKey);
      ...
      if (startState.parent) {
        ...
        for (var i = 0, len = children.length; i < len; i++) {
          if (get.get_1(children[i], keys.rowKey) === startRowValue) {
            targetIndex = i;
            count += 1;
          }

          if (get.get_1(children[i], keys.rowKey) === endRowValue) {
            currentIndex = i;
            count += 1;
          }
    ...
    value: function expandAll(dataSource, keys) {
      ...
      var expandLoop = function expandLoop(dataSource2, keys2) {
        ...
        for (var i = 0, len = dataSource2.length; i < len; i++) {
          ...
          var rowValue = get.get_1(item, keys2.rowKey);

          var state = _this.treeDataMap.get(rowValue);

          var children = get.get_1(item, keys2.childrenKey);
    ...
    value: function initialTreeDataMap(treeDataMap, dataSource, column, keys) {
      ...
      for (var i = 0, len = dataSource.length; i < len; i++) {
        ...
        var state = {
          ...
          expanded: false,
          expandChildrenLength: 0,
          disabled: table_utils.isRowSelectedDisabled(column, item, i),
          parent: parent
        };
    ...
    value: function updateDisabledState(dataSource, column, keys) {
      for (var i = 0, len = dataSource.length; i < len; i++) {
        ...
        var state = this.treeDataMap.get(rowValue);
        state.disabled = table_utils.isRowSelectedDisabled(column, item, i);
        this.treeDataMap.set(rowValue, state);
        var children = get.get_1(item, keys.childrenKey);
...
function updateRowExpandLength(treeDataMap, row, distance, type, keys) {
  ...
  while (tmp) {
    ...
    var state = treeDataMap.get(get.get_1(tmp, keys.rowKey));
...
function clearRowExpandLength(treeDataMap, row, keys) {
  var children = get.get_1(row, keys.childrenKey);

  if (children !== null && children !== void 0 && children.length) {
    children.forEach(function (item) {
      var state = treeDataMap.get(get.get_1(item, keys.rowKey));
      if (!state) return;
      state.expandChildrenLength = 0;
...
function updateRowData(data, key, newData, keys) {
  for (var i = 0, len = data.length; i < len; i++) {
    ...
    if (get.get_1(item, keys.rowKey) === key) {
      data[i] = newData;
      return;
    }
...
function updateRowIndex(treeDataMap, dataSource, extra) {
  ...
  for (var rowIndex = start; rowIndex < end; rowIndex++) {
    ...
    var state = treeDataMap.get(get.get_1(item, extra.rowKey));
