...
function Header(props){
    return(
        <CalendarIcon size='2em' onClick={(e)=>{goto2('/src/pages/calendar.html?type=noteBook',e)}}/>
    )
}
...
function AddButton(props){
    function setPic(){
        var destination='../public/pics/notebook/'
        ...
        axios.post(hostServer+'/users/addNoteBook',{user:user,time:dateT,content:hostServer+'/pics/notebook/'+fileName,tap:1})
    }
...
class Item extends React.Component {
  ...
  render(){
    if(this.state.tap==1){
      return(
        <img className="py-sm w-3/5 ml-xl" src={this.state.content} onClick={eve=>{this.viewPic(this.state.content,eve)}}/>
      )
    }
    else{
      return (
...
class ItemList extends React.Component {
  ...
  componentDidMount(){
    axios.get(hostServer+`/users/notebook?user=${user}&time=${dateT}`).then(e=>{
      this.setState({
        itemList:e.data
      })
    })
  ...
  addWord(){
    ...
    axios.post(hostServer+'/users/addNoteBook',{user:user,time:dateT,content:this.state.value,tap:0})
    this.setState({
        itemList:words,
        addOrNot:false,
        value:''
  ...
  render(){
    var items=this.state.itemList.map((e)=>
      <Item content={e.content} key={e.content} tap={e.tap}/>
    )
    ...
    <div>
      {card}
        <div className='containerA justify-around py-sm mb-sm bg-primaryH'>
            <Header />
            <AddButton2 showCard={this.showCard} />
            <AddButton addPic={this.addPic}/>
        </div>
        <div>
            {items}
        </div>
