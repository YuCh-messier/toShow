...
var props=defineProps(['projectspe','tap','father'])
var emits=defineEmits(['sendprojectspe'])
var {projectspe}=toRefs(props)
...
function save(myid){
    ...
    for(let i=0;i<ifShow.value.length;i++){
        ...
        let fileName=`projectstep2${i}${myid}`
    ...
    emits('sendprojectspe',projectspe.value,myid,tap.value)
}
</script>
